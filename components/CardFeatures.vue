<template>
    <div class="container mx-auto py-16">
        <div class="flex flex-wrap -mx-4">
            <div class="w-1/3 pb-8 px-4">
                <p class="text-center text-5xl text-primary-900 mb-4">
                    <i class="fas fa-dollar-sign"></i>
                </p>
               <p class="text-primary-800 font-semibold text-xl text-center">Compras y ahorros seguros</p>
               <!-- <p class="font-light mt-3 text-center">
                Lorem, ipsum dolor sit amet consectetur adipisicing elit. Commodi ut maxime fugit neque veniam accusantium deleniti? Necessitatibus culpa iure mollitia.
               </p> -->
            </div>
            <div class="w-1/3 pb-8 px-4">
                <p class="text-center text-5xl text-primary-900 mb-4">
                    <i class="fas fa-user-shield"></i>
                </p>
               <p class="text-primary-800 font-semibold text-xl text-center">Compras y ahorros seguros</p>
            </div>
            <div class="w-1/3 pb-8 px-4">
                <p class="text-center text-5xl text-primary-900 mb-4">
                    <i class="fas fa-mobile-alt"></i>
                </p>
               <p class="text-primary-800 font-semibold text-xl text-center">Encuentra el negocio o servicio m{as cercano a tu localidad</p>
            </div>

            <div class="w-1/3 pb-8 px-4">
                <p class="text-center text-5xl text-primary-900 mb-4">
                    <i class="fas fa-globe"></i>
                </p>
               <p class="text-primary-800 font-semibold text-xl text-center">Planes de salud y seguros</p>
            </div>
            <div class="w-1/3 pb-8 px-4">
                <p class="text-center text-5xl text-primary-900 mb-4">
                    <i class="fas fa-paw "></i>
                </p>
               <p class="text-primary-800 font-semibold text-xl text-center">Descuentos y promociones en miles de establecimientos</p>
            </div>
            <div class="w-1/3 pb-8 px-4">
                <p class="text-center text-5xl text-primary-900 mb-4">
                    <i class="far fa-building"></i>
                </p>
               <p class="text-primary-800 font-semibold text-xl text-center">Red de beneficios para tus mascotas y otros miembros de la familia</p>
            </div>
        </div>
    </div>
</template>